  ...
  runInitBuild(isSync) {
    try {
      ...
      if (isSync) {
        ...
        let localPhaser = path.join(paths.base, 'node_modules', 'phaser-ce', 'build', 'phaser.min.js')
        fs.access(localPhaser, fs.constants.F_OK, (err) => {
          if (err && (err.code === 'ENOENT')) {
            fs.copySync(path.join(paths.cli.phaser, 'phaser.min.js'), path.join(paths.game_vendor, 'phaser.min.js'))
          } else {
            fs.copySync(localPhaser, path.join(paths.game_vendor, 'phaser.min.js'))
          }
        })
      }
